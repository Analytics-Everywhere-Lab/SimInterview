from llm_parser import get_llm_output, get_llm_output_with_parser
import pymupdf
...
def structure_cv(cv_text):
    ...
    return get_llm_output(
        temperature=0.3, max_tokens=1500, system_role=system_prompt, prompt=prompt
    )
...
def structure_jd(jd_text):
    ...
    """
    return get_llm_output(
        temperature=0.3, max_tokens=1200, system_role=system_prompt, prompt=prompt
    )
...
"""
    ...
    return get_llm_output(
        temperature=0.3, max_tokens=1200, system_role=system_prompt, prompt=user_prompt
    )
...
def generate_question_bank(cv_text: str, jd_text: str) -> List[str]:
    ...
    raw_output = get_llm_output_with_parser(
        max_tokens=500, system_role=system_prompt, prompt=user_prompt
    )
